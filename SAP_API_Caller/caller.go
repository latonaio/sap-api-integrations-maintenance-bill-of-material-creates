...
import (
	...
	sap_api_output_formatter "sap-api-integrations-maintenance-bill-of-material-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callMaintenanceBillOfMaterialSrvAPIRequirementHeader("A_MaintenanceBillOfMaterialHeader", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaintenanceBillOfMaterialSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	outputDataItem, err := c.callMaintenanceBillOfMaterialSrvAPIRequirementItem("A_MaintenanceBillOfMaterialItem", item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaintenanceBillOfMaterialSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
